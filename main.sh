...
- perubahan output proses bruteforce
- memperbaiki stuck di saat bruteforce
     ...
     # warna
       bi=$(mode.bold: biru)    cy=$(mode.bold: cyan)
		...
		def: brute_zip(){
			...
			var urutkan : 0
			var regex_size : "([0-9]+).$jumlah"

			# rematch
			if [[ $jumlah =~ $regex_size ]]; then
				var size : $(echo "${BASH_REMATCH[1]}" | sed 's; ;'';g')
			fi
			while ((urutkan<=jumlah)); do
				...
				if (unzip -P "$burute" "$1" &> /dev/null); then
					Tulis.strN "\n${ku}[${pu}$(date +%H:%M:%S)${ku}]${st} ${ku}[${ij}INFO${ku}]${me}-${ij}>${ku} [${pu}files${me}:${pu}$1${ku}] [${pu}pass${me}:${pu}$burute${ku}]${un} (${ku}hasil${me}:${ij}berhasil${un})${st}\n"
					println_info " zip berhasil di bobol\n"
					exit 0
				else
					((progress = urutkan * 100 / 80))
					printf "\r${cy}[${ku}•${me}•${cy}]${pu} processing ${me}${urutkan}${m}:${ij}${size} ${me}{${ij}${progress}${me}}${ku} [${ij}INFO${ku}]${pu} mencoba ${me}-${ij}>${pu} $burute${st}"
